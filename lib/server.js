...
Server.prototype._onMessage = function(messageListener, message, connection) {
    if (message instanceof SessionCreateMessage) {
        ...
        this.emit('session', session, connection, session.params, function(err, optionalResponse, clientType) {
            if (err) {
                return session.deny(optionalResponse, clientType);
            }
            session.accept(optionalResponse, clientType);
        });
