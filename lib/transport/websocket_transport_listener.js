...
var component = 'websocketTransportListener';
...
CONST.ESTABLISH_SESSION_TIMEOUT = 10000;

var WebsocketTransportListener = AbstractTransportListener.extend({
    CONST: CONST,
    ...
    init: function(options) {
        ...
        if (options.server) {
            logger.debug(component, 'constructed with server');
            this.server = new ws.Server({server: options.server});
        } else {
            logger.debug(component, 'constructed with port', {port: options.port});
            var app = express();
    ...
    _configure: function() {
        ...
        logger.debug(component, 'configured connection handler');
    },

    _onConnection: function(websocket) {
        var params = {};
    ...
    _paramsExtract: function(object, params) {
    ...
    _onNewSession: function(connection) {
        ...
        }.bind(this), CONST.ESTABLISH_SESSION_TIMEOUT);        
    },

    _onAcceptNewSession: function(connection, session, client, response) {
    ...
    _onDenyNewSession: function(connection, session, client, response) {
        var transport = new WebsocketTransport({client: client, connection: connection});
        client.setTransport(transport);
        client.sendDenySessionMessage(response);
    },

    _onResumeSession: function(connection, sessionToken) {
