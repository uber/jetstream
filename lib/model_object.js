...
ModelObject.prototype._addParent = function(parent, keyPath) {
    if (!(parent instanceof ModelObject) || typeof keyPath !== 'string') {
        throw new Error('Invalid parent or keyPath');
    }
    ...
    if (!this._parentRelationshipsByParentUUID[parent.uuid][keyPath]) {
        this._parentRelationships.push(parentRelationship);
        this._parentRelationshipsByParentUUID[parent.uuid][keyPath] = parentRelationship;
    }
};

ModelObject.prototype._removeParent = function(parent, keyPath) {
    if (!(parent instanceof ModelObject) || typeof keyPath !== 'string') {
        throw new Error('Invalid parent or keyPath');
    }
    ...
    if (!this._parentRelationshipsByParentUUID[parent.uuid][keyPath]) {
        throw new Error('No such parent on keyPath');
    }

    var parentRelationship = this._parentRelationshipsByParentUUID[parent.uuid][keyPath];
    var array = this._parentRelationships;
    array.splice(array.indexOf(parentRelationship), 1);

    delete this._parentRelationshipsByParentUUID[parent.uuid][keyPath];
    ...
    this.getProperties().forEach(function(property) {
        if (!property.isModelObjectType) {
            ...
            if (property.isCollectionType) {
                ...
                results[property.name] = this[property.name].uuid;
            }
        }
    }.bind(this));
    maybeCallback(callback)(null, results);
    console.log('getValues?', results);
    return results;
};
