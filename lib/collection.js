...
Collection.prototype.getModelObjects = function(callback) {
    if (!this.property.isModelObjectType) {
        throw new Error('Collection does not contain ModelObjects');
    }

    async.map(this.array, function(lazyModelObject, doneCallback) {
        lazyModelObject.getModelObject(doneCallback);
    }, callback);
};

Collection.prototype.removeModelObject
