            ...
            async.mapSeries(addSyncFragments, function(syncFragment, doneCallback) {
                ...
                this._applySyncFragmentProperties(syncFragment, modelObject, temporalModelObjectsByUUID, function(err) {
                    if (err) {
                        ...
                        _.each(this._syncFragmentResult(err), function(key, value) {
                            result[key] = value;
                        });
        ...
        function commitAttachedTemporalModelObjects(results, temporalModelObjectsByUUID, addResultsByTemporalModelObjectUUID, nextCallback) {
            ...
            async.mapSeries(modelObjects, function(modelObject, doneCallback) {
                ...
                if (!modelObject.scope) {
                    err = new Error('No scope attached, parent sync fragment failed');
                    _.each(this._syncFragmentResult(err), function(key, value) {
                        result[key] = value;
                    });
                    return doneCallback();
        ...
        if (!property.isModelObjectType) {
            if (property.isCollectionType && !(value instanceof Array)) {
                ...
                async.map(value, function(element) {
                    this._verifyFragmentValueProperty(property, key, element, doneCallback);
                }.bind(this), doneCallback);
            } else {
                this._verifyFragmentValueProperty(property, key, value, doneCallback);
            }
        } else {
            this._verifyFragmentModelObjectProperty(property, key, value, addedModelObjectsByUUID, doneCallback);
        }
    }.bind(this), callback);
};

Scope.prototype._verifyFragmentValueProperty = function(property, key, value, callback) {
    ...
    if (!validNumber && !validString && !validBoolean && !validDate && !validNull) {
        return callback(new Error(
            'Not valid type at \'' + key + '\', should be ' + property.singleType.name));
    }
    callback();
};

Scope.prototype._verifyFragmentModelObjectProperty = function(property, key, value, addedModelObjectsByUUID, callback) {
    if (property.isCollectionType) {
        if (!(value instanceof Array)) {
            return callback(new Error('Not valid type at \'' + key + '\', should be array of UUIDs'));
        }
        var err;
        _.each(value, function(uuid) {
            ...
            if (!uuid || typeof uuid !== 'string') {
                err = new Error('Not valid type at \'' + key + '\', should be array of UUIDs');
            }
        });
        if (err) {
            return callback(err);
        ...
        var notFoundUUIDs = _.filter(value, function(uuid) {
            return !Boolean(addedModelObjectsByUUID[uuid.toLowerCase()]);
        });
        ...
        async.mapSeries(notFoundUUIDs, function(uuid, nextCallback) {
            this.persist.containsModelObjectWithUUID(uuid.toLowerCase(), function(err, result) {
                if (err || !result) {
                    err = err || new Error('No such ModelObject \'' + uuid + '\' at \'' + key + '\'');
                    return nextCallback(err);
                }
                nextCallback();
            });
        ...
        if (typeof value !== 'string') {
            return callback(new Error('Cannot set property at \'' + key + '\' as non-UUID'));
        }
        ...
        this.persist.containsModelObjectWithUUID(value, function(err, result) {
            if (err || !result) {
                err = err || new Error('No such ModelObject \'' + value + '\' at \'' + key + '\'');
                return callback(err);
            }
            callback();
        });
    ...
    async.waterfall([
        ...
        function verifyRemove(modelObject, parentRelationships, nextCallback) {
            ...
            _.each(parentRelationships, function(parentRelationship) {
                ...
                var keyPath = parentRelationship.keyPath;
        ...
        function removeModelObject(modelObject, parentRelationshipsAndFoundIndexes, nextCallback) {            
            this.persist.removeModelObject(modelObject, function(err) {
                ...
                _.each(parentRelationshipsAndFoundIndexes, function(entry) {
                    ...
                    var keyPath = entry.parentRelationship.keyPath;
                    var property = parent.getProperty(keyPath);
